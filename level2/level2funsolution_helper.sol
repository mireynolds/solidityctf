...
contract AddressCalculator {
    function calculateAddresses(address deployingAddress, uint64 nonce) public pure returns (address, address) {
        address contract1 = getAddress(deployingAddress, nonce);
        address contract2 = getAddress(contract1, 1);
        return (contract1, contract2);
    }
    function getAddress(address addr, uint nonce) internal pure returns (address) {
        bytes memory data;
